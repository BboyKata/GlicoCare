...
from src.paziente import Paziente  # <--- Import esplicito in cima
...
# ==========================================================
# GESTIONE TERAPIE CON DATE (CORRETTO PER ELIMINAZIONE)
# ==========================================================
    ...
    def prescriviTerapia(self, id_paz: int, farmaco: str, assunzioniGiornaliere: int, quantita: str, data_inizio: str, data_fine: str = None, indicazioni: str = None):
        """
        Prescrive una nuova terapia con data_inizio specifica e data_fine opzionale.
        """
        conn = sqlite3.connect(self._db_path)
        cursor = conn.cursor()
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """
            cursor.execute(query, (id_paz, farmaco, assunzioniGiornaliere, quantita, indicazioni, self._id_ref, data_inizio, data_fine))
            conn.commit()
            self.registra_operazione("PRESCRIZIONE_TERAPIA", "TERAPIA", f"{id_paz}/{farmaco}", f"{farmaco} - {assunzioniGiornaliere}/die (dal {data_inizio})")
        except sqlite3.Error as e:
            print(f"Errore prescrizione terapia: {e}")
        finally:
            conn.close()
    ...
    def modificaTerapia(self, id_paz: int, vecchio_farmaco: str, nuovo_farmaco: str, assunzioniGiornaliere: int, quantita: str, data_inizio: str, data_fine: str = None, indicazioni: str = None):
        """
        Modifica una terapia: chiude la vecchia (data_fine = OGGI) e crea la nuova con i dati passati.
        """
        conn = sqlite3.connect(self._db_path)
        cursor = conn.cursor()
        try:
            ...
            cursor.execute("UPDATE TERAPIA SET data_fine=? WHERE id_paz=? AND farmaco=? AND data_fine IS NULL", (oggi, id_paz, vecchio_farmaco))
            # 2. Inserisci la nuova terapia
            cursor.execute("""
            INSERT INTO TERAPIA (id_paz, farmaco, assunzioniGiornaliere, quantita, indicazioni, id_med, data_inizio, data_fine)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (id_paz, nuovo_farmaco, assunzioniGiornaliere, quantita, indicazioni, self._id_ref, data_inizio, data_fine))
            conn.commit()
            self.registra_operazione("MODIFICA_TERAPIA", "TERAPIA", f"{id_paz}/{nuovo_farmaco}", f"{vecchio_farmaco}→{nuovo_farmaco} ({assunzioniGiornaliere}/die) dal {data_inizio}")
        except sqlite3.Error as e:
            print(f"Errore modifica terapia: {e}")
        finally:
            conn.close()
...
class Medico:
    ...
    # ==========================================================
    # GESTIONE TERAPIE (LOGICA CORRETTA)
    # ==========================================================
